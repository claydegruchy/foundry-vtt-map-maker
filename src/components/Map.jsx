...
const Map = (props) => {
  ...
  var Poly = ({ points }) => (
    <Shape
      ...
      fill={mapRenderStyles[toolData.style].fillColor}
      stroke={mapRenderStyles[toolData.style].strokeColor}
      strokeWidth={mapRenderStyles[toolData.style].strokeWidth}
      points={points}
    />
  );
    ...
    const L = ({ points }) => {
      ...
      return (
        <Line
          fill={mapRenderStyles[toolData.style].fillColor}
          stroke={mapRenderStyles[toolData.style].strokeColor}
          strokeWidth={mapRenderStyles[toolData.style].strokeWidth / 2}
          points={points}
        />
      );
    };
    ...
    if (features.geometry.type == 'Polygon')
      return (
        <>
          ...
          {toolData.generateInternalDoors &&
            lines.map((l) => <L points={turf.getCoords(l).flat()} />)}
        </>
      );
  ...
  const ConvertLayerToTurfPoly = (children) => {
    ...
    for (var shape of children) {
      transformedShapes.push(
        turf.polygon([getCorners(shape)], { fill: '#0f0' })
      );
    }
  ...
  const convertPolyToScene = ({ poly, intersections }) => {
    ...
    var addWall = (points, center) => {
      ...
      drawings = [
        ...drawings,
        makeDrawing(points, mapRenderStyles[toolData.style]),
      ];
    ...
    return makeScene({
      walls,
      drawings,
      style: mapRenderStyles[toolData.style],
    });
  };
          ...
          <ControlBox>
            <label>
              ...
              <select
                name='select'
                onChange={(e) =>
                  setToolData({ ...toolData, style: e.target.value })
                }
              >
                {Object.keys(mapRenderStyles).map((n) => {
                  return (
                    <option selected={toolData.style === n} value={n}>
                      {n}
                    </option>
                  );
                })}
          ...
          >
            ...
            >
              <Rect
                ...
                width={window.innerWidth / stageParams.scale}
                height={window.innerHeight / stageParams.scale}
                fill={mapRenderStyles[toolData.style].stageBackground}
                id={'background'}
              />
